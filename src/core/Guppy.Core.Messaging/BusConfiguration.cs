...
namespace Guppy.Core.Messaging
{
    public class BusConfiguration
    {
        ...
        public void SetQueue(Type type, int queue, int priority = _defaultPriority)
        {
            this._queues.Add((type, queue, priority));
        }

        public (Type type, int queue)[] GetQueueConfigurations()
        {
            return this._queues.OrderByDescending(x => x.priority).Select(x => (x.type, x.queue)).ToArray();
        }
    }
}